...
export function PoolTabNav({
  ...
  return (
    <div className="sticky top-[56px] -mt-px z-40 w-full min-w-0 max-w-full overflow-hidden pb-2 bg-[rgba(var(--bg-app-rgb),0.86)] supports-[backdrop-filter]:backdrop-blur-md">
      <Surface
        tier={1}
        className="flex w-full min-w-0 max-w-full gap-1 overflow-x-auto rounded-full p-1 no-scrollbar [scrollbar-width:none] [-ms-overflow-style:none] [overscroll-behavior-x:contain] [-webkit-overflow-scrolling:touch]"
      >
        {showMemberTabs && (
          <button onClick={() => onChange("overview")} className={tabButtonClass("overview")}>
            Overview
          </button>
        )}
        <button onClick={() => onChange("rules")} className={tabButtonClass("rules")}>
          Rules
        </button>
        {showMemberTabs && (
          <>
            <button onClick={() => onChange("seats")} className={tabButtonClass("seats")}>
              Seats
            </button>
            <button onClick={() => onChange("members")} className={tabButtonClass("members")}>
            ...
            <button onClick={() => onChange("history")} className={tabButtonClass("history")}>
              History
            </button>
          </>
        )}
        {showOrganizer && (
          <button onClick={() => onChange("organizer")} className={tabButtonClass("organizer")}>
            <span className="inline-flex items-center gap-2">
